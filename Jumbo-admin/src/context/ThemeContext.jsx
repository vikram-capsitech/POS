...
export const ThemeProvider = ({ children }) => {
  ...
  const fetchTheme = async () => {
    try {
      ...
      if (
        profile.restaurant &&
        profile.restaurant.theme &&
        profile.restaurant.theme.primary
      ) {
  ...
  const theme = createTheme({
    ...
    typography: {
      ...
      fontSize: 16, // 1.6rem -> 16px to match index.css var(--font-md)
      htmlFontSize: 10,
    },
    components: {
      MuiButton: {
  ...
  return (
    <ThemeContext.Provider
      value={{ primaryColor, setPrimaryColor, fetchTheme }}
    >
