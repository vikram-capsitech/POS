...
// @route   PUT /api/pos/restaurant/:id
...
// @route   PUT /api/pos/restaurant/:id/theme
router.put('/:id/theme', protect, isAdmin, asyncHandler(async (req, res) => {
    let info = await Restaurant.findById(req.params.id);
    if (!info) {
        res.status(404);
        throw new Error('Restaurant not found');
