import { getAvailableUsersAndChannels } from "../Api";
...
import { SearchIcon } from "../Assets/CustomAntIcons";
import { Avatar } from "antd";
import { useTheme } from "../Contexts/ThemeContext";
import { useEffect, useState } from "react";

export const SearchUserChannelInput = ({
  onChange,
  value,
  placeholder,
  isMulti = false,
  ...
  const _loadSuggestions = (query: string, callback: any) => {
    requestHandler(
      async () =>
        await getAvailableUsersAndChannels(query, currentOrganization?.id),
      null,
      (res) => {
        setReload(false);
        if (res.data) {
          const { users = [], channels = [] } = res.data;
          ...
          const combined = [...userOptions, ...channelOptions];
          callback(combined);
        } else {
          callback([]);
        }
  ...
  useEffect(() => {
    ...
    if (selectAll) {
      requestHandler(
        async () =>
          await getAvailableUsersAndChannels("", currentOrganization?.id),
        null,
        (res) => {
          if (res.data) {
            const { users = [], channels = [] } = res.data;
            ...
            const allOptions = [...userOptions, ...channelOptions];

            setAllUsers(allOptions);
            onChange(isMulti ? allOptions : allOptions[0] || null);
          }
        },
        (error: string) => {
          dispatch(showSnackbar({ severity: "error", message: error }) as any);
        ...
        styles={{
          control: (baseStyles, state) => ({
            ...
            boxShadow: state.isFocused
              ...
              : "none", // 👈 this controls the focus ring
