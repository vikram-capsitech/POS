...
import { fetchMenu, fetchTables, createOrder, addItemsToOrder, updateOrder, updateTable, fetchOrderById } from '@/app/services/api';
import {
  Box,
  Paper,
  Typography,
...
export function WaiterPOS() {
  ...
  const [restaurantInfo, setRestaurantInfo] = useState({ name: 'My Restaurant', address: '', phone: '' });
  ...
  const loadData = async () => {
    try {
      const [tableData, menuData] = await Promise.all([fetchTables(), fetchMenu()]);
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:5000/api/restaurant').then(res => {
  ...
  useEffect(() => {
    const loadTableOrder = async () => {
      if (selectedTable && selectedTable.currentOrderId) {
        try {
          setLoading(true);
          const order = await fetchOrderById(selectedTable.currentOrderId.toString());
  ...
  useEffect(() => {
    const notifyInterval = setInterval(() => {
      if (Math.random() > 0.7) {
        ...
        setNotifications(prev => [`Kitchen: Order for Table ${randomTable} is Ready! 🍳`, ...prev]);
      }
  ...
  const removeItem = (itemId: string) => {
    setCurrentOrder(currentOrder.filter((item) => (item.menuItem.id || (item.menuItem as any)._id) !== itemId));
  };
  ...
  const handleClearTable = async () => {
    ...
    if (window.confirm(`Clear Table ${selectedTable.number}? This will free the table.`)) {
      try {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      if (selectedTable.currentOrderId) {
        ...
        await addItemsToOrder(selectedTable.currentOrderId.toString(), itemsPayload, total);
      } else {
        // Create new order
        const orderData = {
  ...
  const getTableColor = (status: Table['status']): 'success' | 'info' | 'warning' | 'error' | 'primary' => {
    switch (status) {
  ...
  return (
    <Box sx={{ height: 'calc(100vh - 120px)', position: 'relative' }}>
      ...
      <Box sx={{ mb: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        {viewMode === 'menu' ? (
          <Button startIcon={<ArrowLeft />} onClick={() => setSelectedTable(null)}>Back to Tables</Button>
        ) : (
          <Typography variant="h5" fontWeight="bold">Floor Plan</Typography>
        )}

        <Stack direction="row" spacing={2}>
          <Button variant="outlined" startIcon={<Bell />} onClick={() => setOpenNotifications(true)}>
            Notifications
            {notifications.length > 0 && (
              <Badge badgeContent={notifications.length} color="error" sx={{ ml: 1, mb: 1 }} />
            )}
          </Button>
        </Stack>
      </Box>
          ...
          {tables.map((table) => {
            ...
            return (
              <Grid size={{ xs: 6, sm: 4, md: 3 }} key={tableId}>
                ...
                >
                  <CardActionArea sx={{ height: '100%', p: 2 }} onClick={() => setSelectedTable(table)}>
                    <Stack height="100%" justifyContent="space-between">
                      <Box display="flex" justifyContent="space-between">
                        <Typography variant="h4" fontWeight="bold" color={color + ".main"}>{table.number}</Typography>
                        <Chip label={table.status} color={color} size="small" sx={{ textTransform: 'capitalize' }} />
                      </Box>

                      {/* AI Assignment Badge */}
                      <Box sx={{ bgcolor: 'background.paper', p: 1, borderRadius: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Avatar sx={{ width: 24, height: 24, fontSize: '0.7rem' }}>{waiterName[0]}</Avatar>
                        <Box>
                          <Typography variant="caption" color="text.secondary" display="block" lineHeight={1}>Served by</Typography>
                          <Typography variant="caption" fontWeight="bold">{waiterName}</Typography>
                        </Box>
                        <Sparkles size={14} color="#FFD700" style={{ marginLeft: 'auto' }} />
                      </Box>

                      <Typography variant="body2" color="text.secondary" align="center">{table.seats} Seats</Typography>
                    </Stack>
                  </CardActionArea>
                </Card>
              </Grid>
      ...
      {viewMode === 'menu' && (
        <Box sx={{ height: '100%', display: 'flex', gap: 2, overflow: 'hidden' }}>
          {/* Left: Menu Grid */}
          <Paper elevation={0} variant="outlined" sx={{ flex: 1, display: 'flex', flexDirection: 'column', borderRadius: 3, overflow: 'hidden' }}>
            ...
            <Box sx={{ p: 2, flex: 1, overflowY: 'auto', bgcolor: 'background.default' }}>
                        ...
                        >
                          ...
                          <CardContent sx={{ flexGrow: 1, p: 2 }}>
                            ...
                            <Stack direction="row" justifyContent="space-between" alignItems="center" mt={1}>
                              ...
                              <Box sx={{ bgcolor: alpha(theme.palette.primary.main, 0.1), p: 0.5, borderRadius: 2 }}>
          ...
          {/* Right: Order Cart */}
          <Paper elevation={0} variant="outlined" sx={{ width: 400, display: 'flex', flexDirection: 'column', borderRadius: 3, overflow: 'hidden', bgcolor: 'background.paper' }}>
            <Box sx={{ p: 2.5, bgcolor: 'primary.main', color: 'primary.contrastText', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Box>
                <Typography variant="h6" fontWeight="bold">Table {selectedTable?.number}</Typography>
                <Typography variant="caption" sx={{ opacity: 0.9 }}>Order Details</Typography>
              </Box>
              <Chip label="Dine In" size="small" sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: 'white' }} />
            </Box>

            <Box sx={{ flex: 1, overflowY: 'auto', p: 2 }}>
              {currentOrder.length === 0 ? (
                <Box height="100%" display="flex" flexDirection="column" alignItems="center" justifyContent="center" color="text.secondary">
                  <Box sx={{ p: 3, bgcolor: 'action.hover', borderRadius: '50%', mb: 2 }}>
                    <Utensils size={32} />
                  </Box>
                  <Typography>No items in order</Typography>
                  <Typography variant="caption">Tap menu items to add</Typography>
                </Box>
              ) : (
                <Stack spacing={2}>
                  {currentOrder.map((item, index) => {
                    const itemId = item.menuItem.id || (item.menuItem as any)._id; // Robust ID
                    return (
                      <Paper key={index} variant="outlined" sx={{ p: 1.5, borderRadius: 3, display: 'flex', gap: 2, alignItems: 'center' }}>
                        <Avatar variant="rounded" src={item.menuItem.imageUrl} sx={{ width: 56, height: 56, borderRadius: 2 }} />
                        <Box flex={1}>
                          <Typography variant="subtitle2" fontWeight="bold">{item.menuItem.name}</Typography>
                          <Typography variant="caption" color="text.secondary">₹{item.menuItem.price * item.quantity}</Typography>
                        </Box>
                        <Stack direction="row" alignItems="center" spacing={1} sx={{ bgcolor: 'action.hover', borderRadius: 3, p: 0.5 }}>
                          <IconButton size="small" onClick={() => updateQuantity(itemId, -1)} disabled={item.quantity <= 1}>
                            <Minus size={14} />
                          </IconButton>
                          <Typography variant="body2" fontWeight="bold" sx={{ minWidth: 20, textAlign: 'center' }}>{item.quantity}</Typography>
                          <IconButton size="small" onClick={() => updateQuantity(itemId, 1)}>
                            <Plus size={14} />
                          </IconButton>
                        </Stack>
                        <IconButton color="error" size="small" onClick={() => removeItem(itemId)}>
                          <Trash2 size={16} />
                        </IconButton>
                      </Paper>
                    )
              ...
              <Stack spacing={2} mb={2}>
                <Paper variant="outlined" sx={{ p: 2, borderRadius: 3, bgcolor: 'background.paper' }}>
                  <Stack direction="row" justifyContent="space-between" mb={1}>
                    ...
                    <Typography fontWeight="bold">₹{getTotalAmount()}</Typography>
                  </Stack>
                  <Stack direction="row" justifyContent="space-between" mb={1}>
                    <Typography color="text.secondary">Tax (5%)</Typography>
                    <Typography fontWeight="bold">₹{Math.floor(getTotalAmount() * 0.05)}</Typography>
                  </Stack>
                  <Divider sx={{ my: 1 }} />
                  <Stack direction="row" justifyContent="space-between">
                    <Typography variant="h6" fontWeight="bold">Total</Typography>
                    <Typography variant="h6" fontWeight="bold" color="primary">₹{Math.floor(getTotalAmount() * 1.05)}</Typography>
                  </Stack>
                </Paper>
              </Stack>
              ...
              {selectedTable?.currentOrderId ? (
                <Box>
                  ...
                  <Grid container spacing={1} mb={2}>
                    <Grid size={{ xs: 6 }}>
                      <Button variant="outlined" size="small" color="success" fullWidth startIcon={<Smartphone size={16} />} onClick={handlePrintBill}>WhatsApp Bill</Button>
                    </Grid>
                    <Grid size={{ xs: 6 }}>
                      <Button variant="outlined" size="small" color="error" fullWidth startIcon={<XCircle size={16} />} onClick={handleClearTable}>Clear</Button>
                    </Grid>
                  </Grid>
                  <Grid container spacing={1} mb={1}>
                    <Grid size={{ xs: 6 }}>
                      <Button
                        ...
                        sx={{ flexDirection: 'column', py: 1, fontSize: '0.7rem' }}>
                  ...
                  >
                    {selectedTable?.currentOrderId ? "Add to Order" : "Place Order"}
                  </Button>
                </Box>
              ) : (
                <Button
                  ...
                  onClick={handlePlaceOrder}
                  startIcon={<Check />}
                  sx={{ borderRadius: 3, height: 56, fontSize: '1.1rem', boxShadow: theme.shadows[4] }}
                >
      ...
      >
        <Box position="relative" overflow="hidden">
          ...
          >
            ...
            <Typography color="text.secondary" mb={3}>Transaction ID: TXN-{Date.now().toString().slice(-6)}</Typography>

            <Box sx={{ bgcolor: 'warning.light', p: 2, borderRadius: 3, mb: 3, display: 'inline-flex', alignItems: 'center', gap: 1 }}>
              <PartyPopper size={24} color="#B45309" />
              <Typography fontWeight="bold" color="#B45309">You earned {rewardPoints} Points!</Typography>
            </Box>
          </motion.div>

          <Button variant="contained" fullWidth size="large" onClick={() => setOpenRewardDialog(false)} sx={{ borderRadius: 3 }}>
            Done
          </Button>
        </Box>
      </Dialog>
      ...
      <Dialog open={openBillDialog} onClose={() => setOpenBillDialog(false)} maxWidth="xs" fullWidth>
        <DialogTitle>Send Bill via WhatsApp</DialogTitle>
        <DialogContent>
          <Typography variant="body2" color="text.secondary" mb={2}>
            Enter the customer's phone number to send a detailed bill directly to their WhatsApp.
          </Typography>
          <TextField
            autoFocus
            margin="dense"
            ...
            InputProps={{
              startAdornment: <Typography color="text.secondary" mr={1}>+91</Typography>
            }}
          />
          <Box mt={2}>
            <Typography variant="subtitle2" fontWeight="bold">Bill Preview:</Typography>
            <Box sx={{ p: 2, bgcolor: 'grey.100', borderRadius: 2, mt: 1, fontSize: '0.75rem', fontFamily: 'monospace', whiteSpace: 'pre-wrap' }}>
              {restaurantInfo.name}
              {"\n----------------\n"}
        ...
        <DialogActions>
          ...
          <Button onClick={sendWhatsAppBill} variant="contained" color="success" startIcon={<Send size={16} />}>
            Send Bill
          </Button>
        </DialogActions>
      </Dialog>
